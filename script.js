const formatador = (data) => {
  ...
  const atividade = {
    nome: "Chegada",
    data: new Date("2024-07-08 05:30"),
    finalizada: true 
  }
  ...
  let atividades = [
    ...
    {
      nome: 'Saída',
      data: new Date("2024-07-13 06:00"),
      finalizada: false
    }
  ]
  ...
  const criarHorasSelecao = () => {
    ...
    for(let i = 6; i < 23; i++) {
      ...
      horasDisponiveis += `<option value"${hora}:00>${hora}:00</options>`
      horasDisponiveis += `<option value"${hora}:30>${hora}:30</options>`
    }
